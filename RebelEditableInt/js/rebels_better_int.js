    ...
    async beforeRegisterNodeDef(nodeType, nodeData, app) {
        if (nodeData.name === "RebelsBetterIntNode") {
            ...
            nodeType.prototype.onNodeCreated = function() {
                ...
                // Expand default size to accommodate the 3 native ComfyUI widgets + display area
                this.size = [300, 250];
            };

            // Keep the right-click rename just for the top physical Title Bar of the node
            ...
            // Dynamic Foreground Drawing (The Scalable UI)
            const onDrawForeground = nodeType.prototype.onDrawForeground;
            nodeType.prototype.onDrawForeground = function(ctx) {
                onDrawForeground?.apply(this, arguments);

                // COLLAPSED BUG FIX: Stops display from rendering outside a closed node
                if (this.flags.collapsed) return;

                // Pull values directly from the native ComfyUI widgets you interact with
                ...
                const displayName = nameWidget && nameWidget.value ? nameWidget.value : "Value";
                ...
                // Draw Glowing Dynamic Number
                ctx.fillStyle = theme.text;
                ctx.font = `bold ${dynamicFontSize}px monospace`;
                ctx.textAlign = "center";
                ctx.shadowColor = theme.text;
                ctx.shadowBlur = dynamicFontSize * 0.3; // Glow radius scales with the text
                
                // Vertically center the number in the remaining available box space
                const numberY = startY + dynamicNameSize + 20 + (dynamicFontSize * 0.7);
                ctx.fillText(displayValue, w / 2, numberY);
            };

            // Dynamic Custom Background Rendering
            const onDrawBackground = nodeType.prototype.onDrawBackground;
            nodeType.prototype.onDrawBackground = function(ctx) {
                onDrawBackground?.apply(this, arguments);
                
                // COLLAPSED BUG FIX: Stops effects from bleeding onto the graph
                if (this.flags.collapsed) return;
